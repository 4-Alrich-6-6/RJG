...
(function () {
  ...
  async function fetchReportedJobs(supa, userId) {
    const { data } = await supa
      .from("content_report")
      .select("target_job_id")
      .eq("reporter_id", userId)
      .eq("target_type", "job");
    return new Set((data || []).map(r => r.target_job_id).filter(Boolean));
  }
  ...
  async function getRecommendations({ mode = "" } = {}) {
    ...
    const [
      ...
      prefs, interactions, applied, bookmarks, hidden, reported, rawJobs, dropdownMap
    ] = await Promise.all([
    ...
    const stage1 = rawJobs
      .filter(j => !hidden.has(j.id) && !reported.has(j.id))
      .map(rawJob => {
