...
ALTER TABLE public.user_profile 
  ...
  async function uploadJobImage(file, jobId) {
    ...
    const path = `${jobId || genRandomId()}.${ext}`;
    ...
    try {
      ...
      if (!signedErr && signedData && signedData.signedUrl) {
        console.log("[uploadJobImage] Got signed URL:", signedData.signedUrl.substring(0, 100) + "...");
        return signedData.signedUrl;
      }
      if (signedErr) console.warn("[uploadJobImage] Signed URL error:", signedErr);
    } catch (e) {
      console.warn("[uploadJobImage] Signed URL failed, falling back:", e);
    }
    const { data: urlData } = supa.storage.from("job-images").getPublicUrl(path);
    console.log("[uploadJobImage] Public URL:", urlData?.publicUrl);
    return urlData && urlData.publicUrl ? urlData.publicUrl : "";
  }

  async function getJobImageUrl(imageUrlOrPath) {
    if (!imageUrlOrPath) return "";
    // If it's already a full URL with signed token, return it
    if (String(imageUrlOrPath).includes("?token=")) return imageUrlOrPath;
    // If it's a full URL but not signed, try to create a signed URL
    ...
    // It's just a path, create signed URL
    const supa = getClient();
    if (!supa) return imageUrlOrPath;
    const { data, error } = await supa.storage
      .from("job-images")
      .createSignedUrl(imageUrlOrPath, 60 * 60 * 24 * 7);
    if (!error && data && data.signedUrl) return data.signedUrl;
    // Fall back to public URL
    const { data: urlData } = supa.storage.from("job-images").getPublicUrl(imageUrlOrPath);
